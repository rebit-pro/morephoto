...
#[AsCommand(
    name: 'app:identity:sync:consume',
    description: 'Консьюмер очереди синхронизации identity',
)]
#[WithLock]
final class IdentitySyncConsumerCommand extends RebitCommand
{
    private const int DEFAULT_LIMIT = 100;
    private const int DEFAULT_TIME_LIMIT = 300;

    public function __construct(
        private readonly ConsumeIdentitySyncUseCase $consumeUseCase,
    ) {
        parent::__construct();
    }
    ...
    protected function handle(SymfonyStyle $io, InputInterface $input): int
    {
        ...
        $io->info(sprintf('Запуск consumer identitySync (limit=%d, time-limit=%d сек)', $limit, $timeLimit));
