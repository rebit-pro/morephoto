...
#[AsCommand(
    name: 'app:exchange:chat-script:consume',
    description: 'Консьюмер очереди шагов чат-скриптов',
)]
#[WithLock]
final class ChatScriptStepConsumerCommand extends RebitCommand
{
    private const int DEFAULT_LIMIT = 100;
    private const int DEFAULT_TIME_LIMIT = 300;

    public function __construct(
        private readonly ConsumeChatScriptStepsUseCase $consumeUseCase,
    ) {
        parent::__construct();
    }
    ...
    protected function handle(SymfonyStyle $io, InputInterface $input): int
    {
        ...
        $io->info(sprintf('Запуск consumer chatScriptStep (limit=%d, time-limit=%d сек)', $limit, $timeLimit));
