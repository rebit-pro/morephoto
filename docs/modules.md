...
Для минимального фронтового сценария поддерживается одношаговый скрипт, который может содержать:
...
| `steps[].message` | string | ✅ | Текст сообщения или комментарий к вложению |
| `steps[].delaySeconds` | int | ✅ | Задержка перед отправкой |
| `steps[].contentType` | string | — | `str` / `pic` / `pdf` / `video` |
| `steps[].fileName` | string\|null | — | Имя загруженного файла |
| `steps[].fileUrl` | string\|null | — | URL или data URL вложения |
...
3. Пользователь создаёт одношаговый сценарий с текстом или вложением.
